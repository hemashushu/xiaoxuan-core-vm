...
use ancvm_thread::{resizeable_memory::ResizeableMemory, thread::Thread};

pub fn heap_capacity(thread: &mut Thread) {
    // `fn () -> pages:i64`
    let pages = thread.heap.get_capacity_in_pages();
    thread.stack.push_i64_u(pages as u64);
}

pub fn heap_resize(thread: &mut Thread) {
...
pub fn heap_fill(thread: &mut Thread) {
...
pub fn heap_copy(thread: &mut Thread) {
